...
export default function MaterialBom({ orders }: { orders: Order[] }) {
  ...
  const products = useMemo(() => {
    const m = new Map<string, string>(); prodOrders.forEach(o => { if (!m.has(o.name)) m.set(o.name, o.spec); });
    let arr = [...m.entries()].map(([name, spec]) => ({ name, spec }));
    if (q.trim()) arr = arr.filter(p => (p.name + p.spec).toLowerCase().includes(q.toLowerCase()));
      ...
      <div className="card">
        ...
        <div style={{ overflow: "auto", maxHeight: "45vh" }}>
          <table style={{ borderCollapse: "collapse", width: "100%" }}>
            <thead><tr>{["품목", "규격", "AgCN (50g당, g)", "PGC (50g당, g)", "비고"].map(h => <th key={h} style={{ ...TH, textAlign: h.includes("g)") ? "right" : "left" }}>{h}</th>)}</tr></thead>
            <tbody>
              {products.length === 0 ? <tr><td colSpan={5} style={{ ...TD, textAlign: "center", color: "#888", padding: 24 }}>주문에 등록된 제품이 없습니다.</td></tr> :
