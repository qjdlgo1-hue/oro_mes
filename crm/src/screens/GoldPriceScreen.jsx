...
import { parseGoldPaste, monthlyAvg } from "../lib/gold";
...
export function GoldPriceScreen({ canEdit = true }) {
  ...
  // ----- 붙여넣기 → 미리보기 → 저장 (엑셀 이력 / 신한은행 형식 자동 인식) -----
  const doPreview = () => setPreview(parseGoldPaste(paste, year));
  const previewCount = preview ? preview.excelRows.length + preview.shinhanRows.length : 0;
  const doSave = async () => {
    if (!preview || previewCount === 0) return;
    setSaving(true);
    try {
      // 엑셀 형식: PGC·구매 기록까지 전체 열 저장 (같은 날짜는 전체 덮어씀)
      if (preview.excelRows.length > 0) await goldPricesUpsert(preview.excelRows);
      // 신한은행 형식: 시세만 갱신 — 직접 입력한 PGC·구매 기록은 보존
      if (preview.shinhanRows.length > 0) await goldPricesUpsert(preview.shinhanRows);
      setPaste(""); setPreview(null);
      await load();
      alert(`${previewCount}일치 시세를 저장했습니다.`);
  ...
  // 전체 선택은 표에 보이는 행(최근 120일) 기준
  const allSelected = list.length > 0 && selected.size > 0 && selected.size >= Math.min(list.length, 120);
  const toggleSelectAll = () => setSelected(allSelected ? new Set() : new Set(list.slice(0, 120).map((r) => r.date)));
  ...
  const SHOW_ROWS = 120; // 표에는 최근 120일만 (전체 이력은 월 평균 계산에 사용)
  const shown = list.slice(0, SHOW_ROWS);
  ...
  return (
    <div style={{ padding: isMobile ? 14 : 28 }}>
      {/* 붙여넣기 입력 */}
      {canEdit && (
        <Panel title="시세 붙여넣기 — 엑셀 이력 / 신한은행 표 자동 인식">
          <div style={{ display: "flex", gap: 10, alignItems: "flex-start", flexWrap: "wrap" }}>
            <div style={{ flex: 1, minWidth: 260 }}>
              <textarea
                style={{ ...inputStyle, minHeight: 96, resize: "vertical", fontSize: 12 }}
                placeholder={"두 형식 모두 그대로 붙여넣으면 됩니다 (섞여 있어도 자동 판별).\n① 엑셀 이력: 2023-06-30  80,972  536  ...  (PGC·구매 기록까지 전체 저장)\n② 신한은행: 07.20.  191,251.69  상승1,019.13  +0.54%  ...  (시세만 갱신)"}
                value={paste}
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              <div>
                <div style={{ fontSize: 11, fontWeight: 700, color: T.sub, marginBottom: 4 }}>연도 (신한은행 형식용)</div>
              ...
              <button onClick={doSave} disabled={!preview || previewCount === 0 || saving} style={{ ...btnStyle("primary"), opacity: preview && previewCount > 0 && !saving ? 1 : 0.4 }}>
                {saving ? "저장 중..." : `저장${preview ? ` (${previewCount}일)` : ""}`}
              </button>
            </div>
          </div>
          {preview && (
            <div style={{ marginTop: 10 }}>
              <div style={{ fontSize: 12, fontWeight: 700, color: previewCount ? T.ok : T.danger }}>
                {previewCount}일치 인식됨
                {preview.excelRows.length > 0 && ` — 엑셀 이력 ${preview.excelRows.length}일(전체 열 저장)`}
                {preview.shinhanRows.length > 0 && ` — 신한은행 ${preview.shinhanRows.length}일(시세만 갱신)`}
                {preview.errors.length ? ` · 실패 ${preview.errors.length}건: ${preview.errors.join(", ")}` : ""}
              </div>
              {previewCount > 0 && (
                <div style={{ fontSize: 11, color: T.sub, marginTop: 4 }}>
                  {[...preview.excelRows, ...preview.shinhanRows].slice(0, 8).map((r) => `${r.date} 기준율 ${won(r.close)}`).join(" · ")}{previewCount > 8 ? ` 외 ${previewCount - 8}일` : ""}
                  <span style={{ marginLeft: 6 }}>— 엑셀 형식은 같은 날짜의 PGC·구매 기록까지 덮어쓰고, 신한은행 형식은 시세만 갱신합니다</span>
                </div>
              )}
            </div>
          )}
      ...
      <div style={{ background: T.card, borderRadius: 12, border: `1px solid ${T.border}`, overflow: "hidden" }}>
        <div style={{ padding: "14px 20px", borderBottom: `1px solid ${T.border}`, display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
          <div style={{ fontWeight: 700, fontSize: 15, marginRight: "auto" }}>
            일별 시세·구매 기록 <span style={{ fontSize: 12, color: T.sub, fontWeight: 600 }}>(전체 {list.length}일 중 최근 {shown.length}일 표시)</span>
          </div>
